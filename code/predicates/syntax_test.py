...
def test_term_substitute(debug=False):
    ...
    result = Term.parse('h(c,f(x))').substitute(substitution_map, {'y'})
    if debug:
...
def test_free_variables(debug=False): # Add formulae with logical operators
    for s,expected_variables in [
...
def test_propositional_skeleton(debug=False):
    from predicates.util import fresh_variable_name_generator
